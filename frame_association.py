...
def cal_dist(left_matrix: np.ndarray, right_matrix: np.ndarray, weight=0.9):
    if (left_matrix == right_matrix).all():
        return pdist(left_matrix, "euclidean") * weight + pdist(right_matrix, "cosine") * (1-weight)
...
def get_camera_bias(frames):
    ...
    for sensor in embeddings_under_sensor:
        ...
        dist = cal_dist(embeddings, embeddings)
        # Exclude noise
